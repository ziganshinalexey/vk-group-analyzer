...
(function($) {
    ...
    function handleClick(event, container, options) {
        ...
        if (1 < event.which || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) {
            return;
        }

        // Ignore cross origin links
        if (link.protocol !== location.protocol || location.hostname !== link.hostname) {
            return;
        }

        // Ignore case when a hash is being tacked on the current URL
        if (-1 < link.href.indexOf('#') && stripHash(link) == stripHash(location)) {
            return;
        }
    ...
    function handleSubmit(event, container, options) {
        ...
        var defaults = {
            ...
            url: $form.attr('action') + hash,
            container: $form.attr('data-pjax'),
            target: form
        };

        if ('GET' !== defaults.type && window.FormData !== undefined) {
    ...
    function pjax(options) {
        ...
        options.error = function(xhr, textStatus, errorThrown) {
            ...
            if ('GET' == options.type && 'abort' !== textStatus && allowed) {
        ...
        options.success = function(data, status, xhr) {
            ...
            var currentVersion = ('function' === typeof $.pjax.defaults.version) ?
    ...
    function locationReplace(url) {
        ...
        // Если есть хэш, то страница не обновится. Обновить страницу принудительно.
        if (hash) {
            setTimeout(function() {
    ...
    function extractContainer(data, xhr, options) {
        ...
        if (options.fragment) {
            ...
            if ($fragment.length) {
                obj.contents = 'body' === options.fragment ? $fragment : $fragment.contents();
    ...
    if ($.event.props && 0 > $.inArray('state', $.event.props)) {
